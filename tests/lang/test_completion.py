...
def test_completion_loc():
    completions = bel.lang.completion.bel_completion('a(CHEBI:water, loc(cytoskeleton', bel_fmt='medium')
    print('Completions:\n', json.dumps(completions, indent=4))
    assert completions["completion_text"] == "cytoskeleton"
    assert ["a(CHEBI:water, loc(GO:cytoskeleton"] == [c['replacement'] for c in completions["completions"] if c['replacement'] == "a(CHEBI:water, loc(GO:cytoskeleton"]
...
@pytest.mark.skip(reason="Skip for now - need to add partialparse error checks for this and return error messages")
def test_completion_error_1():

    completions = bel.lang.completion.bel_completion('act(complex ()', cursor_loc=12, bel_fmt='medium')
    print('Completions:\n', json.dumps(completions, indent=4))
    assert completions["completion_text"] == "AKT2"
    assert completions['completions'][0]['replacement'] == "act(r(fus(HGNC:AKT2"
    assert completions['completions'][0]['cursor_loc'] == 19
...
def test_completion_complex_fn_1():
    ...
    print('Completions', json.dumps(completions, indent=4))
    assert ["complex(p(HGNC:EGFL6), loc()"] == [c['replacement'] for c in completions["completions"] if c['replacement'] == "complex(p(HGNC:EGFL6), loc()"]
